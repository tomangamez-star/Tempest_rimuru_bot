...
function systemPrompt(staff = false) {
  ...
  const lbStr = lb.length
    ...
    : 'No players yet';
  const memCtx = memory.contextString(3);
  const base = [
...
async function reply(text, user) {
  ...
  // Remember pattern: "remember that <key> is <value>" or "remember <key> = <value>"
  ...
  // Recall pattern: "what do you know about <topic>" or "recall <topic>"
  ...
  if (recallMatch) {
    ...
    if (mem) return `I remember: ${mem.value}`;
