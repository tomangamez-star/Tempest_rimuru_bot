...
- Card archive: automatic external artwork spawning is retired. /cshop sells T1–T6 cards from the Telegram Shoob archive, and one paid archive offer appears hourly. /char and /shoob search archived originals only.
- Collections: /collection and /wlb show legacy waifus; /characters, /viewchar and /clb show purchased archive cards. /hunt, /shunt, /card, /waifu and /swaifu redirect to /cshop.
- Shoob originals: /shoob <name> [T1-T6] searches the pre-ingested Telegram archive and offers Previous/Next buttons. /cstats shows the live catalogue size, media/tier distribution, scraper progress, latest-run results, speed and ETA. Owner-only /scrapestart checks GitHub run state and immediately requests ingestion when no run is queued or active. Shoob media is stored by Telegram; Supabase stores metadata/file_ids only. /cardstyle option 4 makes /char use Shoob originals.
