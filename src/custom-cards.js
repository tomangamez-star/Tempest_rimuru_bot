...
async function upload(cardId,buffer){ if(!ready())throw new Error('SUPABASE_SERVICE_ROLE_KEY is not configured'); await ensureBucket(); const path=`custom/${cardId}.png`,url=`${projectUrl()}/storage/v1/object/${BUCKET}/${path}`; const r=await fetch(url,{method:'POST',headers:{Authorization:`Bearer ${key()}`,apikey:key(),'Content-Type':'image/png','x-upsert':'true'},body:buffer}); if(!r.ok)throw new Error(`Supabase Storage HTTP ${r.status}: ${(await r.text()).slice(0,120)}`); return path; }
