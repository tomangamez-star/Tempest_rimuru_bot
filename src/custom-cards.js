...
async function upload(cardId,buffer,options={}){ if(!ready())throw new Error('Supabase card storage key is not configured (SUPABASE_SERVICE_ROLE_KEY or SUPABASE_SECRET_KEY)'); await ensureBucket(); const animated=options.animated===true||String(options.contentType||'').includes('video'); const ext=animated?'mp4':'png',contentType=animated?'video/mp4':'image/png'; const path=`custom/${cardId}.${ext}`,url=`${projectUrl()}/storage/v1/object/${BUCKET}/${path}`; const r=await fetch(url,{method:'POST',headers:{Authorization:`Bearer ${key()}`,apikey:key(),'Content-Type':contentType,'x-upsert':'true'},body:buffer}); if(!r.ok)throw new Error(`Supabase Storage HTTP ${r.status}: ${(await r.text()).slice(0,120)}`); return path; }
...
function setOverride(cardId,ownerId){const c=get(cardId);if(!c||/\.mp4$/i.test(c.storage_path||''))return null; return db.setCardOverride({override_key:overrideKey(c.name,c.tier),card_id:c.card_id,name:c.name,tier:c.tier,renderer:c.renderer,set_by:ownerId,created_at:Date.now()});}
