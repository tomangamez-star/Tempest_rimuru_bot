...
const TelegramBot = require("node-telegram-bot-api"),
  ...
  db = require("./db"),
  eco = require("./economy"),
  cd = require("./cooldowns"),
  admin = require("./admin"),
  leaderboard = require("./leaderboard"),
  rimuru = require("./rimuru"),
  memory = require("./memory"),
  income = require("./income"),
  { fmt, humanDuration, note, pick, esc, THEME } = require("./utils"),
  send = require("./send"),
  ...
  higherlower = require("./games/higherlower"),
  lottery = require("./games/lottery"),
  race = require("./games/race"),
  guess = require("./games/guess"),
  crash = require("./games/crash"),
  wheel = require("./games/wheel"),
  rps = require("./games/rps"),
  tictactoe = require("./games/tictactoe"),
  dicevs = require("./games/dicevs"),
  cfstreak = require("./games/cfstreak"),
  numroulette = require("./games/numroulette"),
  shop = require("./shop"),
  crime = require("./crimes/crime"),
  fishing = require("./fish"),
  robbery = require("./crimes/robbery"),
  heist = require("./crimes/heist"),
...
try {
  commitHash =
    ...
    require("child_process")
      .execSync("git rev-parse --short HEAD", { timeout: 2e3 })
      .toString()
      .trim();
} catch (e) {
  commitHash = null;
}
const REACT_KEYS = Object.keys(config.reactions).filter(
    (k) => k !== "fallback",
  ),
  MENU_COMMANDS = [
    ...
    {
      command: "purge",
      description: "💀 Purge balances (mods request, owner approves)",
    },
    ...
    { command: "cset", description: "👑 Set official custom card" },
    { command: "creset", description: "👑 Reset official card" },
    { command: "char", description: "🃏 Search a character card" },
    { command: "cardstyle", description: "🎴 Choose /char card style" },
...
function createBot() {
  ...
  staffSpectator.attach(bot);
  const depsPhoto = (chatId, imageUrl, opts) =>
  ...
  async function reply(chatId, text, opts = {}) {
    const trustedHtml = opts.html === !0 || opts.parse_mode === "HTML";
    if (
      ...
      return send.sendText(bot, chatId, safe, { ...opts, html: !0, raw: !0 });
    }
  ...
  async function editMsg(chatId, messageId, text, opts = {}) {
    ...
    if (opts.raw) {
      const safe = send.sanitizeHtml(text);
      return send.editText(bot, chatId, messageId, safe, {
        ...opts,
        html: !0,
        raw: !0,
      });
    }
    return send.editText(bot, chatId, messageId, text, {
      ...opts,
      html: trustedHtml,
    });
  }
  ...
  function buildCtx(msg, args = []) {
    ...
    return {
      ...
      reply: (t, o) =>
        reply(msg.chat.id, t, {
          ...(msg._replyTarget ? { _replyTarget: msg._replyTarget } : {}),
          ...o,
        }),
      ...
      answerCb: (text) =>
        bot
          .answerCallbackQuery(
            text && text.query_id ? text.query_id : void 0,
  ...
  function parseCommand(text) {
    ...
    return { cmd: parts[0].split("@")[0].toLowerCase(), args: parts.slice(1) };
  }
  function repliedUser(msg) {
    const r = msg.reply_to_message;
    return !r || !r.from ? null : r.from;
  }
  function canInteract(userId, gambling = !0) {
  ...
  async function nextSticker() {
    ...
    try {
      return (
        stickerCache ||
          (stickerCache = (
            (await bot.getStickerSet(config.stickerPack)).stickers || []
          ).map((s) => s.file_id)),
        stickerCache.length
          ? stickerCache[Math.floor(Math.random() * stickerCache.length)]
          : ((stickerDisabled = !0), null)
      );
    } catch (e) {
      return (
        console.warn(
          `[sticker] pack "${config.stickerPack}" unavailable — stickers disabled:`,
          e.message,
        ),
        (stickerDisabled = !0),
        null
  ...
  async function stickerAfterText(chatId, textPromise) {
    ...
    try {
      ...
      return (console.warn("[sticker] send failed:", e.message), null);
    }
  }
  const membershipCache = new Map(),
    MEMBERSHIP_CACHE_MAX = 5e3;
  function cacheMembership(userId, val) {
    if (
      (membershipCache.set(userId, val),
      membershipCache.size > MEMBERSHIP_CACHE_MAX)
    ) {
      const oldest = membershipCache.keys().next().value;
      oldest !== void 0 && membershipCache.delete(oldest);
    }
  }
            ...
            try {
              ...
              chat && chat.id
                ? ((resolvedGroupId = chat.id),
                  console.log(
                    `[gate] required group ${config.requiredGroup} resolved to chat ${resolvedGroupId}`,
                  ))
                : console.warn(
                    `[gate] getChat(${config.requiredGroup}) returned no id — gate OFF`,
                  );
            } catch (e) {
              console.warn(
                `[gate] could not resolve ${config.requiredGroup}: ${e.message} — gate OFF until resolved`,
              );
            } finally {
              groupResolvePromise = null;
            }
  ...
  async function checkMembershipFresh(userId) {
    if (isStaff(userId)) return { ok: !0, staff: !0 };
    const gid = await resolveRequiredGroup();
    if (!gid) return { ok: !0, gateOff: !0 };
    try {
      const m = await bot.getChatMember(gid, userId),
        status = m && m.status,
        member = ["creator", "administrator", "member"].includes(status);
      return (
      ...
      return (
        console.warn(
          `[gate] getChatMember(${gid}, ${userId}) error: ${e.message}`,
        ),
        { ok: !1, status: "error" }
      );
    }
  ...
  async function gateAllowed(userId) {
    ...
    return cached && Date.now() - cached.at < config.groupGateCacheMs
      ? { ok: cached.ok, cached: !0 }
      : checkMembershipFresh(userId);
  }
  async function gateActive() {
    return config.requiredGroup
      ? config.requiredGroupId
        ? !0
        : !!(await resolveRequiredGroup())
      : !1;
  }
...
Welcome to the house, ${ctx.msg.from.first_name || "mortal"}. Everything is unlocked. 🎰`,
        { title: "✅ VERIFIED", color: THEME.gold, html: !0 },
      );
    else {
  ...
  function targetsRimuru(msg) {
    ...
    if (text.startsWith("/") || rimuru.shouldTrigger(text)) return !0;
    const r = msg.reply_to_message;
    return !!(r && r.from && r.from.is_bot === !0);
  }
  function reactionFor(text) {
    const t = String(text || "").toLowerCase();
    for (const key of REACT_KEYS)
      if (t.includes(key)) return config.reactions[key];
    return /\b(win|won|rich|profit|gain|lucky|jackpot|nice|great|love|good|yay)\b/.test(
      t,
    )
      ? pick(["🎉", "🔥", "💯", "😄", "💰"])
      : /\b(lose|lost|broke|bad|sad|cry|rip|ouch|damn|fail|unlucky)\b/.test(t)
        ? pick(["💸", "😢", "😅", "🫠", "🙃"])
        : /\b(angry|mad|hate|rage|wtf|fuck|annoying)\b/.test(t)
          ? pick(["😡", "🤬", "😤"])
          : t.includes("?")
            ? pick(["🤔", "🧐"])
            : pick(REACT_POOL);
...
Pick 1-10. Rarer picks pay more — up to 9x. 🎡`,
  ...
  async function sendMenu(chatId, page = "main", opts = {}) {
    ...
    return reply(chatId, m.text, {
      ...
      icon: "📜",
      html: !0,
      reply_markup: send.inlineMarkup(m.markup),
      ...opts,
    });
  }
  async function editMenu(chatId, messageId, page = "main") {
    ...
    return editMsg(chatId, messageId, m.text, {
      ...
      html: !0,
      reply_markup: send.inlineMarkup(m.markup),
    }).catch((e) => (console.warn("[menu] edit failed:", e.message), null));
  }
  const charStylePending = new Map();
  function charStyleKey(userId) {
    return `char_style:${userId}`;
  }
  function getCharStyle(userId) {
    const v = String(db.getSetting(charStyleKey(userId)) || "").toLowerCase();
    return ["gen2", "oldgen", "signature", "shoob"].includes(v) ? v : "";
  }
  async function promptCharStyle(ctx, query = "") {
    charStylePending.set(Number(ctx.userId), {
      query: String(query || ""),
      chatId: ctx.chatId,
      at: Date.now(),
    });
    return ctx.reply(
      "🎴 <b>CHARACTER VIEW STYLE</b>\n\nWhich card style would you like Rimuru to use for /char?\n\n<b>1️⃣ Hunt — Gen 2</b>\n<b>2️⃣ SHunt — Old Gen</b>\n<b>3️⃣ Card — JTF Signature ♦️</b>\n<b>4️⃣ Shoob Original 🎴</b>\n\nReply <code>1</code>, <code>2</code>, <code>3</code>, or <code>4</code>. Rimuru remembers this until you use /cardstyle again.",
      { title: "🎴 CARD STYLE", color: THEME.gold, html: !0 },
    );
  }
  async function handleCharLookup(ctx) {
    const q = (ctx.args || []).join(" ").trim();
    if (!q)
      return ctx.reply(
        "Usage: <code>/char &lt;name&gt;</code> or <code>/whois &lt;name&gt;</code>",
        { title: "🃏 CARDS", color: THEME.gold, html: !0 },
      );
    const style = getCharStyle(ctx.userId);
    if (!style) return promptCharStyle(ctx, q);
    const r = await hunt.searchAndShow(q, { chatId: ctx.chatId, style });
    r.ok ||
...
Tap a button below to explore — or just type a command. 👇`,
        ...
        config.showReplyKeyboard)
      )
        try {
          await bot.sendMessage(
            ctx.chatId,
            "⌨️ Your quick-menu keyboard is ready below.",
            { reply_markup: keyboards.keyboardFor("main") },
          );
        } catch (e) {
          console.warn("[start] keyboard:", e.message);
        }
      if (!isStaff(ctx.userId) && !(await gateAllowed(ctx.userId)).ok) {
        const p = gatePrompt(ctx.chatId);
        await ctx.reply(p.text, p.opts);
      }
    },
    ...
    casino: async (ctx) => {
      await ctx.reply(MENU.casino().text, {
        ...
        color: THEME.cyan,
        html: !0,
        reply_markup: config.showReplyKeyboard
          ? keyboards.keyboardFor("casino")
          : send.inlineMarkup(MENU.casino().markup),
      });
    },
    games: async (ctx) => {
      await ctx.reply(MENU.games().text, {
        ...
        html: !0,
        reply_markup: config.showReplyKeyboard
          ? keyboards.keyboardFor("games")
          : send.inlineMarkup(MENU.games().markup),
      });
    },
    economy: async (ctx) => {
      await ctx.reply(MENU.economy().text, {
        ...
        html: !0,
        reply_markup: config.showReplyKeyboard
          ? keyboards.keyboardFor("economy")
          : send.inlineMarkup(MENU.economy().markup),
      });
    },
    help: async (ctx) => {
      await ctx.reply(
...
• /lottery [tickets] — 1B each, 5 unique buyers = automatic draw
...
• /sb [amount] all · /purge [amount] — owner-only global balance purge
• /mod (reply) — owner appoints a persistent bot moderator
...
• /card — JTF Signature: PSD-faithful adaptive card (owner only)
• /crender — custom renderer: Gen 2 / Old Gen / JTF Signature / AI Custom
...
• /clb [n] — top card collectors (default 10, max 100)
...
💬 <i>Reply to me or say "Rimuru" to talk.</i>`,
        { title: "❓ RIMURU HELP", color: THEME.gold, html: !0 },
      );
    },
    balance: async (ctx) => {
      const u = eco.ensure(ctx.userId, metaOf(ctx.msg));
...
💎 Net worth: <b>${fmt(u.wallet + u.bank)}</b>`,
        {
          title: `💰 ${u.first_name || "YOUR"} BALANCE`,
          color: THEME.gold,
          html: !0,
        },
      );
...
Use <code>/dep [amount|all]</code> to deposit or <code>/wd [amount|all]</code> to withdraw.`,
        { title: "🏦 BANK", color: THEME.cyan, html: !0 },
      );
    },
    income: async (ctx) => {
      await ctx.reply(
...
• /daily — 24h reward
• /bonus — weekly reward`,
        { title: "💵 INCOME", color: THEME.gold, html: !0 },
      );
    },
    dep: async (ctx) => {
    ...
    donate: async (ctx) => {
      ...
      if (!target)
        return ctx.reply(
          "Reply to someone with <code>/donate [amount]</code>. 🎯",
          { title: "💝 DONATE", color: THEME.cyan, html: !0 },
        );
    ...
    transfer: async (ctx) => {
      ...
      if (!target)
        return ctx.reply(
          "Reply to someone with <code>/transfer [amount]</code>. 🎯",
          { title: "🏦 TRANSFER", color: THEME.cyan, html: !0 },
        );
    ...
    rob: async (ctx) => {
      const target = repliedUser(ctx.msg);
      if (!target)
        return ctx.reply("Reply to someone with <code>/rob</code>. 🎯", {
          title: "🕹️ ROBBERY",
          color: THEME.red,
          html: !0,
        });
      ...
      if (g.blocked)
        return ctx.reply(g.message, { title: "🕹️ ROBBERY", color: THEME.red });
      const r = robbery.attempt(ctx.userId, target.id, metaOf(ctx.msg));
      (r.ok && cd.start(ctx.userId, "rob", config.cooldowns.rob),
        await ctx.reply(r.message, { title: "🕹️ ROBBERY", color: THEME.red }));
    },
    heist: async (ctx) => {
      const target = repliedUser(ctx.msg);
      if (!target)
        return ctx.reply("Reply to someone with <code>/heist</code>. 🎯", {
          title: "🏦 HEIST",
          color: THEME.red,
          html: !0,
        });
      ...
      if (g.blocked)
        return ctx.reply(g.message, { title: "🏦 HEIST", color: THEME.red });
    ...
    crime: async (ctx) => {
      ...
      if (g.blocked)
        return ctx.reply(g.message, { title: "🕵️ CRIME", color: THEME.red });
      const r = crime.commit(ctx.userId, ctx.args[0], metaOf(ctx.msg));
      (r.ok && cd.start(ctx.userId, "crime", config.cooldowns.rob),
        await ctx.reply(r.message, {
          title: r.ok ? (r.success ? "✅ CRIME" : "🚔 CRIME") : "🕵️ CRIME",
          color: r.ok && r.success ? THEME.gold : THEME.red,
          html: !0,
        }));
    },
    join: async (ctx) => {
      ...
      if (r.ok && r.message) {
        const open = db.getOpenHeists(),
          fullCrew = open.some(
            (o) => o.members.length >= config.heist.maxMembers,
          );
        if (
          (await ctx.reply(r.message, {
            title: "🤝 JOIN HEIST",
          ...
          const full = open.find(
            (o) => o.members.length >= config.heist.maxMembers,
          );
          if (full) {
            const res = heist.execute(full.leader_id),
              timer = heistTimers.get(full.leader_id);
    ...
    daily: async (ctx) => {
      const r = income.daily(ctx.userId, metaOf(ctx.msg));
      await ctx.reply(r.message, { title: "📅 DAILY", color: THEME.gold });
    },
    bonus: async (ctx) => {
      const r = income.bonus(ctx.userId, metaOf(ctx.msg));
      await ctx.reply(r.message, { title: "🎁 BONUS", color: THEME.gold });
    },
    beg: async (ctx) => {
      const r = income.earn(ctx.userId, "beg", metaOf(ctx.msg));
      await ctx.reply(r.message, { title: "🙏 BEG", color: THEME.cyan });
    },
    work: async (ctx) => {
      const r = income.earn(ctx.userId, "work", metaOf(ctx.msg));
      await ctx.reply(r.message, { title: "💼 WORK", color: THEME.cyan });
    },
    fish: async (ctx) => {
      const r = fishing.fish(ctx.userId, metaOf(ctx.msg));
      await ctx.reply(r.message, { title: "🎣 FISH", color: THEME.cyan });
    },
    dig: async (ctx) => {
      const r = income.earn(ctx.userId, "dig", metaOf(ctx.msg));
      await ctx.reply(r.message, { title: "⛏️ DIG", color: THEME.cyan });
    },
    shop: async (ctx) => {
      await ctx.reply(shop.shopList(), {
        title: "🛒 RIMURU'S SHOP",
        color: THEME.gold,
        html: !0,
      });
    ...
    buy: async (ctx) => {
      ...
      if (!id)
        return ctx.reply(
          "🎯 Usage: <code>/buy [id] [qty]</code> — e.g. <code>/buy 4</code> or <code>/buy hook 2</code>. Check <code>/shop</code> first.",
          { title: "🛒 BUY", color: THEME.red, html: !0 },
        );
      const r = shop.buyItem(ctx.userId, id, ctx.args[1], metaOf(ctx.msg));
      await ctx.reply(r.message, {
        title: r.ok ? "✅ PURCHASED" : "🛒 BUY",
        color: r.ok ? THEME.gold : THEME.red,
        html: !0,
      });
    },
    inv: async (ctx) => {
      await ctx.reply(shop.inventoryText(ctx.userId), {
        title: "📦 INVENTORY",
        color: THEME.cyan,
        html: !0,
      });
    ...
    id: async (ctx) => {
      await ctx.reply(profile.idCardText(ctx, ctx.userId), {
        title: "🪪 ID CARD",
        color: THEME.gold,
        html: !0,
      });
    ...
    lb: async (ctx) => {
      await ctx.reply(leaderboard.render(), {
        title: "🏆 LEADERBOARD",
        color: THEME.gold,
        html: !0,
      });
...
👑 You are at the TOP rank.`
    ...
    ranks: async (ctx) => {
      await ctx.reply(rank.ranksList(), {
        title: "🏆 RANK LADDER",
        color: THEME.gold,
        html: !0,
      });
    ...
    collection: async (ctx) => {
      ...
      await ctx.reply(waifu.collectionCaption(rows), {
        title: "💝 WAIFU",
        color: "#FF80AB",
        html: !0,
      });
    ...
    wlb: async (ctx) => {
      ...
      await ctx.reply(waifu.leaderboardCaption(rows, 100), {
    ...
    character: async (ctx) => {
      const rows = db.getUserCharacters(ctx.userId);
      await ctx.reply(waifu.characterCaption(rows), {
        title: "💝 WAIFU",
        color: "#FF80AB",
        html: !0,
      });
    ...
    customcards: async (ctx) => {
      const rows = customCards.list(ctx.userId);
      if (!rows.length)
        return ctx.reply(
          "You have no saved custom cards yet. Use <code>/crender</code> to make one.",
          { title: "♦️ CUSTOM CARDS", color: THEME.gold, html: !0 },
        );
      await ctx.reply(
        rows
          .slice(0, 30)
          .map(
            (r, i) =>
              `${i + 1}. <b>${r.name}</b> • T${r.tier} • ${r.renderer === "oldgen" ? "Old Gen" : r.renderer === "signature" ? "JTF Signature" : r.renderer === "ai" ? "JTF AI Custom" : "Gen 2"} • <code>#${r.card_id}</code>`,
          )
          .join("\n"),
        { title: "♦️ CUSTOM CARDS", color: THEME.gold, html: !0 },
      );
    ...
    cview: async (ctx) => {
      const id = String((ctx.args || [])[0] || "").replace(/^#/, "");
      const card = customCards.get(id);
      if (
        !card ||
        (!ctx.isOwner && Number(card.user_id) !== Number(ctx.userId))
      )
        return ctx.reply("Custom card not found.", {
    ...
    cset: async (ctx) => {
      if (!ctx.isOwner)
        return ctx.reply(
          "Only the King can replace Rimuru's official cards. 👑",
          { title: "👑 CARD OVERRIDE", color: THEME.red },
        );
      const replied = String(
        ...
        src = [...(ctx.args || []), replied].join(" "),
        m = src.match(/#?(JTF[A-Z0-9]+)/i);
      let cardId = m && m[1];
      if (!cardId && replied) {
        const tm = replied.match(/(?:•|\b)\s*T([1-6])\b/i),
          nm = replied.match(
            /(?:T[1-6]\s*•\s*|JTF CARD RENDERER[^\n]*\n[^\n]*•\s*T[1-6]\s*•\s*)([^\n•]+)/i,
          );
        if (tm && nm) {
          const c = customCards.findLatestByNameTier(
            nm[1].trim(),
            Number(tm[1]),
          );
          if (c) cardId = c.card_id;
        }
      }
      if (!cardId)
        return ctx.reply(
          "Reply to a saved custom-card image/message with <code>/cset</code>, or use <code>/cset #JTF...</code>.",
          { title: "👑 CARD OVERRIDE", color: THEME.gold, html: !0 },
        );
      ...
      await ctx.reply(
        `👑 <b>OFFICIAL CARD OVERRIDE SET</b>\n\n${o.name} T${o.tier} now uses <code>#${o.card_id}</code>.`,
        { title: "👑 CARD OVERRIDE", color: THEME.gold, html: !0 },
      );
    },
    creset: async (ctx) => {
      if (!ctx.isOwner)
        return ctx.reply("Only the King can reset official cards. 👑", {
          title: "👑 CARD OVERRIDE",
          color: THEME.red,
        });
      const a = ctx.args || [],
        tm = String(a.at(-1) || "").match(/^t([1-6])$/i);
      if (!tm || a.length < 2)
        return ctx.reply("Usage: <code>/creset Goku t5</code>", {
          title: "👑 CARD OVERRIDE",
          color: THEME.gold,
          html: !0,
        });
      const name = a.slice(0, -1).join(" ");
      customCards.resetOverride(name, +tm[1]);
      await ctx.reply(
        `♻️ Official override removed for <b>${name} T${tm[1]}</b>. Rimuru will use automatic artwork again.`,
        { title: "👑 CARD OVERRIDE", color: THEME.gold, html: !0 },
      );
    },
    cancel: async (ctx) => {
      if (crender.cancel(ctx.chatId, ctx.userId))
        return ctx.reply("❌ Card render cancelled.", {
          title: "♦️ CARD RENDERER 🛠️",
          color: THEME.gold,
        });
      return ctx.reply("There is no active Card Renderer session to cancel.", {
        title: "♦️ CARD RENDERER 🛠️",
        color: THEME.gold,
      });
    },
    ...
    clb: async (ctx) => {
      const n = Math.min(
          100,
          Math.max(1, parseInt((ctx.args || [])[0], 10) || 10),
        ),
        rows = db.getHuntLeaderboard(n);
      await ctx.reply(hunt.leaderboardCaption(rows, n), {
    ...
    remember: async (ctx) => {
      if (!ctx.isOwner)
        return ctx.reply("Only the King can store memories. 👑", {
          title: "🧠 MEMORY",
          color: THEME.red,
        });
      const args = ctx.args || [];
      if (args.length < 2)
        return ctx.reply("Usage: <code>/remember key value</code>", {
          title: "🧠 MEMORY",
          color: THEME.cyan,
          html: !0,
        });
    ...
    recall: async (ctx) => {
      if (!ctx.isOwner)
        return ctx.reply("Only the King can recall memories. 👑", {
          title: "🧠 MEMORY",
          color: THEME.red,
        });
      const key = (ctx.args || [])[0];
      if (!key)
        return ctx.reply("Usage: <code>/recall key</code>", {
          title: "🧠 MEMORY",
          color: THEME.cyan,
          html: !0,
        });
      ...
      mem
        ...
        : await ctx.reply(`🧠 No memory found for "${key}."`, {
            title: "🧠 MEMORY",
            color: THEME.cyan,
            html: !0,
          });
  ...
  async function handleBroadcast(ctx) {
    ...
    if (!isStaff(ctx.userId))
      return ctx.reply("Only the King and his moderators can broadcast. 👑", {
        title: "📣 BROADCAST",
        color: THEME.red,
      });
...
Owner may broadcast any message. Moderators must keep it relevant to the Rimuru bot (economy, games, events, commands, etc.).`,
        { title: "📣 BROADCAST", color: THEME.cyan, html: !0 },
      );
    if (!ctx.isOwner) {
      ...
      if (!verdict.ok)
        return (
          db.logActivity(
            "mod",
            `Rejected broadcast from ${actor.username || ctx.userId}: ${text.slice(0, 80)} (${verdict.reason})`,
            { target: ctx.userId },
          ),
          ctx.reply(
...
That message isn't relevant to the Rimuru bot (${verdict.reason}). Moderators can only broadcast about the bot, its economy, games, events, commands, or maintenance.`,
            { title: "📣 BROADCAST", color: THEME.red, html: !0 },
          )
        );
    }
    ...
    (dashboard.queueBroadcast(rec.id, rec.message, "all"),
      db.logActivity(
        "broadcast",
        `Broadcast queued by ${actor.username || ctx.userId}: ${text.slice(0, 60)}`,
        { broadcast_id: rec.id },
      ),
      db.logAudit(
        ctx.userId,
        actor.username || String(ctx.userId),
        "broadcast",
        0,
        "target=all",
      ),
...
Delivering to all known users & groups…`,
        { title: "📣 BROADCAST", color: THEME.gold, html: !0 },
      ));
  }
  async function handleSet(ctx) {
    const actor = metaOf(ctx.msg);
    if (!isStaff(ctx.userId))
      return ctx.reply("Only the King and his moderators can set events. 👑", {
        title: "🎯 SET EVENT",
        color: THEME.red,
      });
...
The event goes live immediately (players use <code>/mission [id]</code>) and is announced to all chats.`,
        { title: "🎯 SET EVENT", color: THEME.cyan, html: !0 },
      );
    let type = "mission",
      rest = args;
    const first = args.split(/\s+/)[0].toLowerCase();
    if (
      (broadcastMod.EVENT_TYPES.includes(first) &&
        ((type = first), (rest = args.slice(first.length).trim())),
      !rest)
    )
      return ctx.reply(
        "Give the event a title: <code>/set mission Heist Rimuru | description | 100000</code>",
        { title: "🎯 SET EVENT", color: THEME.red, html: !0 },
      );
    const parts = rest.split("|").map((p) => p.trim()),
      title = parts[0] || "New event",
      description = parts[1] || "",
      reward = Math.max(
        0,
        Math.floor(Number(String(parts[2] || "").replace(/,/g, "")) || 0),
      ),
      ev = db.createEvent({
        title,
        description,
    ...
    (db.logActivity("event", `Event created via /set: ${title} (${type})`, {
      event_id: ev.id,
    }),
      db.logAudit(
        ctx.userId,
    ...
    try {
      ...
      (dashboard.queueBroadcast(rec.id, rec.message, "all"),
        db.logActivity("broadcast", `Event announced via /set: ${ev.title}`, {
          broadcast_id: rec.id,
          event_id: ev.id,
        }));
    } catch (e) {
...
Players use <code>/missions</code> and <code>/mission ${ev.id}</code>.`,
      { title: "🎯 SET EVENT", color: THEME.gold, html: !0 },
    );
  }
  async function handleAttack(ctx) {
    if (!ctx.isOwner)
      return ctx.reply("Only Rimuru (the King) can trigger an attack. 🐉👑", {
        title: "🐉 ATTACK",
        color: THEME.red,
      });
    ...
    if (replied) {
      const raw = String((ctx.args || [])[0] || "").trim(),
        n = Math.floor(Number(raw.replace(/,/g, "")));
      if (!Number.isFinite(n) || n < 1)
        return ctx.reply(
          "Usage: reply to a user with <code>/attack [number]</code> — e.g. <code>/attack 100</code>.",
          { title: "🐉 ATTACK", color: THEME.red, html: !0 },
        );
      const r2 = await attack.deployAgainst(replied.id, n, {
        chatId: ctx.chatId,
        actorId: ctx.userId,
      });
      r2 &&
        !r2.ok &&
        (await ctx.reply(r2.message, {
          title: "🐉 ATTACK",
          color: THEME.red,
          html: !0,
        }));
    ...
    const r = await attack.trigger({
      manual: !0,
      force: !0,
      chatId: ctx.chatId,
      actorId: ctx.userId,
    });
    (r && r.message && !r.targetId) ||
      (r &&
        !r.ok &&
        (await ctx.reply(r.message, {
          title: "🐉 ATTACK",
  ...
  async function handleFbi(ctx) {
    if (!ctx.isOwner)
      return ctx.reply("Only the King can deploy the FBI. 🚔👑", {
        title: "🚔 FBI",
        color: THEME.red,
      });
    const replied = repliedUser(ctx.msg);
    if (!replied)
      return ctx.reply(
        ...
        { title: "🚔 FBI", color: THEME.red, html: !0 },
      );
    const r = await fbi.deployAgainst(replied.id, {
      chatId: ctx.chatId,
      actorId: ctx.userId,
    });
    r &&
...
${res.message}`,
          { title: "🏦 HEIST RESULT", color: THEME.red, html: !0 },
        );
      }
  ...
  async function viewWaifu(ctx) {
    const n = parseInt((ctx.args || [])[0], 10),
      row = db.getUserCharacterByIndex(ctx.userId, n);
    if (!row)
      return ctx.reply(
    ...
    await ctx.reply(waifu.detailCaption(row), {
      title: "💝 WAIFU",
      color: "#FF80AB",
      html: !0,
    });
  ...
  async function viewChar(ctx) {
    const n = parseInt((ctx.args || [])[0], 10),
      row = db.getHuntCharacterByIndex(ctx.userId, n);
    if (!row)
      return ctx.reply(
        "No card at that number. Use <code>/characters</code> to see your list.",
        { title: "🃏 CARDS", color: THEME.gold, html: !0 },
      );
  ...
  async function onCallbackQuery(query) {
    ...
    const data = String(query.data || ""),
      chatId =
        (_b = (_a = query.message) == null ? void 0 : _a.chat) == null
          ? void 0
          : _b.id,
      messageId = (_c = query.message) == null ? void 0 : _c.message_id,
      from = query.from || {},
      userId = from.id,
    ...
    if (pinfo.configured && !pinfo.writable) {
      try {
        await bot.answerCallbackQuery(query.id, {
          text: "Rimuru is in safe maintenance mode while the database recovers.",
          show_alert: !0,
        });
      } catch (_) {}
      return;
    ...
    const answerCb = (text) =>
      ...
      editMsgCb = (text, opts = {}) => editMsg(chatId, messageId, text, opts),
    ...
    try {
      if (data === "gate:verify") {
        (await checkMembershipFresh(userId)).ok
          ? (await editMsg(
              chatId,
              messageId,
              `✅ <b>VERIFIED!</b>
...
Welcome to the house, ${from.first_name || "mortal"}. Everything is unlocked. 🎰`,
      ...
      if (data.startsWith("menu:")) {
        ...
        if (
          ...
          (await editMenu(chatId, messageId, page), await answerCb(""));
          return;
        }
        if (page === "lb") {
          (await replyThreaded(chatId, messageId, leaderboard.render(), {
            title: "🏆 LEADERBOARD",
            color: THEME.gold,
            html: !0,
          }),
        ...
        if (page === "bal") {
          ...
          (await replyThreaded(
            ...
            { title: "💰 BALANCE", color: THEME.gold, html: !0 },
          ),
            await answerCb(""));
          return;
        }
...
💬 <i>Reply to me or say "Rimuru" to talk.</i>`,
        ...
        if (page === "g" && parts[2] && GAME_USAGE[parts[2]]) {
          (await replyThreaded(chatId, messageId, GAME_USAGE[parts[2]], {
            title: "🎮 GAME",
            color: THEME.cyan,
            html: !0,
          }),
        ...
        if (page === "eco") {
          ...
          let r;
          (action === "depAll"
            ? (r = eco.deposit(userId, "all"))
            : action === "dep100k"
              ? (r = eco.deposit(userId, "100000"))
              : action === "wdAll"
                ? (r = eco.withdraw(userId, "all"))
                : action === "wd100k" && (r = eco.withdraw(userId, "100000")),
            r &&
              (await replyThreaded(chatId, messageId, r.message, {
                title: "💼 ECONOMY",
                color: THEME.cyan,
              })),
            await answerCb(""));
  ...
  const PAUSED_NOTICE =
    ...
    pausedNotified = new Set();
  function isPausedFor(msg) {
    if (!db.getBotPaused()) return !1;
    const from = msg.from || {};
    if (String(from.id) === String(config.ownerId)) return !1;
    const parsed = parseCommand(String(msg.text || msg.caption || ""));
    if (parsed && PAUSE_EXEMPT_CMDS.includes(parsed.cmd)) return !1;
  ...
  const PERSISTENCE_EXEMPT_CMDS = new Set([
      "health",
      "debug",
      "help",
      "start",
    ...
    PERSISTENCE_NOTICE =
      "🛠️ Rimuru is temporarily in safe maintenance mode while the database connection recovers. Your balance and progress are protected. Please try again shortly.";
  ...
  async function onMessage(msg) {
    ...
    const text = String(msg.text || msg.caption || ""),
      userId = msg.from.id,
      chatId = msg.chat.id;
    const earlyCtx = buildCtx(msg, []);
    if (await admin.handlePurgeApprovalMessage(earlyCtx, text)) return;
    if (admin.isPurgeLocked()) {
      const purgeCtx = buildCtx(msg, []);
      await admin.handlePurgeMessage(purgeCtx, text);
      return;
    }
    try {
      attack.markSeen(userId);
    } catch (e) {}
    try {
      if (await staffSpectator.checkAndStripLeak(msg, isStaff(userId))) return;
    } catch (e) {
      console.warn("[staff-preview] leak guard:", e.message);
    }
    ...
    db.expirePenalties();
    const check = canInteract(userId, !0);
    if (!check.allowed) {
      check.reply &&
        text.startsWith("/") &&
        (await reply(chatId, check.reply, {
          title: "⛔ BLOCKED",
          color: THEME.red,
        }));
      return;
    ...
    if (
      pendingStyle &&
      Date.now() - pendingStyle.at < 10 * 60 * 1000 &&
      !text.startsWith("/")
    ) {
          ...
          sm[0] === "1"
            ...
            : sm[0] === "2"
              ...
              : sm[0] === "3"
                ? "signature"
                : "shoob";
        db.setSetting(charStyleKey(userId), style);
        charStylePending.delete(Number(userId));
        await reply(
          chatId,
          `✅ Character view style set to <b>${style === "gen2" ? "Hunt — Gen 2" : style === "oldgen" ? "SHunt — Old Gen" : style === "signature" ? "Card — JTF Signature ♦️" : "Shoob Original 🎴"}</b>.`,
          { title: "🎴 CARD STYLE", color: THEME.gold, html: !0 },
        );
        if (pendingStyle.query) {
          const r = await hunt.searchAndShow(pendingStyle.query, {
            chatId,
            style,
          });
          r.ok ||
    ...
    const activeRender = crender.has(chatId, userId),
      renderCommand = parseCommand(text);
    if (activeRender && (!renderCommand || renderCommand.cmd === "cancel")) {
      if (await crender.handleMessage(msg, { bot, reply, eco, db })) return;
    }
    ...
    if (parsed) {
      ...
      if (handler) {
        ...
        try {
          ...
          if (
            !isStaff(ctx.userId) &&
            !["start", "help", "verify"].includes(cmd) &&
            !staffCmds.includes(cmd) &&
            !(await gateAllowed(ctx.userId)).ok
          ) {
          ...
          (console.error(`[cmd /${cmd}] error:`, e.message, e.stack),
            ...
            await reply(
              chatId,
              `⚠️ Something went wrong with /${cmd}. Try again.`,
              { title: "💥 ERROR", color: THEME.red },
            ));
        }
        return;
      }
    ...
    if (kbRoute) {
      ...
      if (kbRoute.page) {
        ...
        await reply(chatId, pageTexts[kbRoute.page] || "", {
          ...
          html: !0,
          reply_markup: config.showReplyKeyboard
            ? keyboards.keyboardFor(kbRoute.page)
            : void 0,
        });
        return;
      }
      ...
      if (handler) {
        ...
        try {
          ...
          (console.error(`[kb /${kbRoute.cmd}] error:`, e.message, e.stack),
            await reply(
              chatId,
              "⚠️ Something went wrong with that button. Try again.",
              { title: "💥 ERROR", color: THEME.red },
            ));
        }
        return;
    ...
    try {
      ...
      console.error("[attack] handleInput error:", e.message);
    }
    try {
      if (await fbi.handleInput(userId, chatId, text)) return;
    } catch (e) {
      console.error("[fbi] handleInput error:", e.message);
    }
    const isReplyToBot =
      msg.reply_to_message &&
      msg.reply_to_message.from &&
      msg.reply_to_message.from.is_bot === !0;
    if (rimuru.shouldTrigger(text) || isReplyToBot) {
      if (!isStaff(userId) && !(await gateAllowed(userId)).ok) {
        const p = gatePrompt(chatId);
        await reply(chatId, p.text, p.opts);
        return;
      }
      ...
      try {
        ...
        (console.error("[rimuru] reply error:", e.message),
          ...
          await reply(
            chatId,
            "Hmph. The void ate my words. Try again, mortal.",
            {
              title: "🐉 RIMURU",
              color: THEME.gold,
              reply_to_message_id: msg.message_id,
  ...
  bot
    ...
    .then((cmds) => {
      console.log(
        `✰ Persistent command menu registered (setMyCommands): ${cmds.map((c) => `/${c.command}`).join(" ")}`,
      );
    })
    .catch((e) => {
  ...
  setInterval(() => {
    try {
      ...
      for (
        let item = dashboard.drainBroadcastQueue(sender);
        item && (drained++, !(drained >= 50));
        item = dashboard.drainBroadcastQueue(sender)
      );
      drained && console.log(`[dashboard] drained ${drained} broadcast(s)`);
    } catch (e) {
  ...
  function makeBroadcastSender() {
    return (item, done) => {
      ...
      let list = [...chats];
      (target === "groups"
        ? (list = list.filter((c) => c < 0))
        : target === "users" && (list = list.filter((c) => c > 0)),
        (list = [...new Set(list)].slice(0, target === "all" ? 1e3 : 500)),
        (async () => {
          ...
          (done(count),
            console.log(
              `[dashboard] broadcast #${item.id} (${target}) delivered to ${count}/${list.length} chats`,
            ));
        })().catch(() => done(count)));
    };
  }
  ...
  (setInterval(() => {
    ...
    setInterval(() => {
      try {
        ...
        console.error("[attack] sweep error:", e.message);
      }
    }, 5e3),
    setInterval(() => {
      try {
        ...
        console.error("[fbi] sweep error:", e.message);
      }
    }, 5e3),
    setInterval(() => {
      try {
        waifu.expireIfNeeded();
      } catch (e) {
        console.error("[waifu] sweep error:", e.message);
      }
    }, 3e4),
    setInterval(() => {
      try {
        ...
        console.error("[hunt] sweep error:", e.message);
      }
    }, 3e4),
    setInterval(() => {
      try {
        ...
        console.error("[rank] time-wallet sweep error:", e.message);
      }
    }, 6e4));
  ...
  return (
    setInterval(() => {
      try {
        ...
        if (nowPeak && !lastPeak) {
          const rec = db.createBroadcast(
            `🌞 <b>PEAK HOURS STARTED</b>

Every game is now a flat 50/50 until 11:00 WAT. Good luck, mortals!`,
            "all",
            Number(config.ownerId),
          );
          (dashboard.queueBroadcast(rec.id, rec.message, "all"),
            console.log(
              "[rank] peak hours STARTED (08:00 WAT) — flat 50/50 engaged",
            ));
        } else if (!nowPeak && lastPeak) {
          const rec = db.createBroadcast(
            `🌙 <b>PEAK HOURS ENDED</b>

Rank-based win chances are back. Top ranks face worse odds — the house protects its whales.`,
            "all",
            Number(config.ownerId),
          );
          (dashboard.queueBroadcast(rec.id, rec.message, "all"),
            console.log(
              "[rank] peak hours ENDED (11:00 WAT) — rank-tier odds restored",
            ));
        }
        lastPeak = nowPeak;
      } catch (e) {
        console.error("[rank] peak-hour scheduler error:", e.message);
      }
    }, 6e4),
