...
// User rows are the critical durable state (wallet/bank/networth + rank/stats).
// Keep a numeric millisecond updated_at so Postgres/SQLite can safely compare
// versions and so every economy/stat mutation can be mirrored promptly.
function userTimestamp(row) {
  if (!row) return 0;
...
function queueUserMirror(userId) {
  if (!PG_CONFIGURED) return;
  try {
    const row = prep('SELECT ' + TABLE_COLS.users + ' FROM users WHERE user_id = ?').get(userId);
    if (!row) return;
    const colNames = TABLE_COLS.users.split(', ').map((c) => c.split(' ')[0]);
    queuePgWrite('users', colNames.map((c) => row[c]));
  } catch (e) {
    // SQLite mutation must not fail just because the optional mirror queue fails.
  }
}
...
function getOrCreateUser(userId, meta = {}) {
  ...
  if (row) {
    ...
    if (updates.length) {
      prep(`UPDATE users SET ${updates.join(', ')}, updated_at = ? WHERE user_id = ?`).run(...params, Date.now(), userId);
      queueUserMirror(userId);
    }
    return prep('SELECT * FROM users WHERE user_id = ?').get(userId);
  }
  const now = Date.now();
  prep(
    'INSERT INTO users (user_id, username, first_name, wallet, bank, networth, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
  ).run(userId, meta.username || '', meta.first_name || '', config.startBalance, 0, config.startBalance, now, now);
  queueUserMirror(userId);
  return prep('SELECT * FROM users WHERE user_id = ?').get(userId);
}
...
function addWallet(userId, amount) {
  prep('UPDATE users SET wallet = wallet + ?, networth = networth + ?, updated_at = ? WHERE user_id = ?').run(amount, amount, Date.now(), userId);
  queueUserMirror(userId);
}

function addBank(userId, amount) {
  prep('UPDATE users SET bank = bank + ?, networth = networth + ?, updated_at = ? WHERE user_id = ?').run(amount, amount, Date.now(), userId);
  queueUserMirror(userId);
}

function setNetworth(userId, amount) {
  prep('UPDATE users SET wallet = ?, bank = 0, networth = ?, updated_at = ? WHERE user_id = ?').run(amount, amount, Date.now(), userId);
  queueUserMirror(userId);
}

function setWallet(userId, amount) {
  prep('UPDATE users SET wallet = ?, networth = bank + ?, updated_at = ? WHERE user_id = ?').run(amount, amount, Date.now(), userId);
  queueUserMirror(userId);
}

function setBank(userId, amount) {
  prep('UPDATE users SET bank = ?, networth = wallet + ?, updated_at = ? WHERE user_id = ?').run(amount, amount, Date.now(), userId);
  queueUserMirror(userId);
}
...
function setRankStats(userId, rank, validMatches, losses) {
  prep('UPDATE users SET rank = ?, rank_valid_matches = ?, rank_consecutive_losses = ?, updated_at = ? WHERE user_id = ?')
    .run(rank, validMatches || 0, losses || 0, Date.now(), userId);
  queueUserMirror(userId);
}
...
function setStatus(userId, status, reason, until = 0) {
  prep('UPDATE users SET status = ?, status_until = ?, status_reason = ?, updated_at = ? WHERE user_id = ?')
    .run(status, until, reason || '', Date.now(), userId);
  queueUserMirror(userId);
}

function clearStatus(userId) {
  prep('UPDATE users SET status = \'\', status_until = 0, status_reason = \'\', updated_at = ? WHERE user_id = ?').run(Date.now(), userId);
  queueUserMirror(userId);
}
...
function expirePenalties() {
  ...
  const expired = [];
  for (const r of rows) {
    prep('UPDATE users SET status = \'\', status_until = 0, status_reason = \'\', updated_at = ? WHERE user_id = ?').run(Date.now(), r.user_id);
    queueUserMirror(r.user_id);
...
function setHidden(userId, until) {
  prep('UPDATE users SET hidden_until = ?, updated_at = ? WHERE user_id = ?').run(until, Date.now(), userId);
  queueUserMirror(userId);
}
...
async function mirrorTable(table) {
  ...
  try {
    ...
    for (const row of rows) {
      // User balances/stats must be versioned. Never allow an older local row
      // to overwrite a newer durable Postgres row during reconciliation.
      if (table === 'users') {
        try {
          const current = await pgPool.query('SELECT updated_at FROM users WHERE user_id = $1', [row.user_id]);
          if (current.rows[0] && userTimestamp(current.rows[0]) > userTimestamp(row)) continue;
        } catch (e) {
          pgWritesFailed++;
          pgLastError = e;
          continue;
...
async function startRecoveryLoop() {
  ...
  pgRecoveryTimer = setInterval(async () => {
    ...
    try {
      ...
      // A connection outage can leave SQLite ahead of or behind durable PG.
      // Re-hydrate/merge before re-enabling writes so recovery itself cannot
      // turn a transient outage into a stale-state overwrite.
      const h = await hydrateFromPg();
      if (!h.enabled) throw new Error('Postgres recovery hydration failed');

      // Revalidate the writer fence after reconnect. The previous advisory-lock
      // session may have been lost with the dead connection.
      if (!pgLockHeld) {
        const lock = await acquireInstanceLock(PG_LOCK_KEY);
        if (!lock) throw new Error('Postgres advisory lock not reacquired');
      }
      ...
      pgDegraded = false;
      pgLastError = null;
      console.log(`[db] Postgres recovered and rehydrated (${h.hydrated} rows)`);
    } catch (e) {
      pgLastError = e;
      pgConnected = false;
      pgReady = false;
  ...
  try {
    for (const table of Object.keys(TABLE_COLS)) {
      ...
      try {
        ...
        if (table === 'users') {
          const insert = prep(`INSERT OR REPLACE INTO ${table} (${colNames.join(', ')}) VALUES (${colNames.map(() => '?').join(', ')})`);
          for (const row of res.rows) {
            const local = prep('SELECT * FROM users WHERE user_id = ?').get(row.user_id);
            // Postgres is authoritative on a fresh Render filesystem. When a
            // surviving SQLite cache exists, however, keep whichever user row
            // has the newer updated_at timestamp so a stale snapshot cannot
            // roll a newer wallet/stat state backwards.
            if (local && userTimestamp(local) > userTimestamp(row)) continue;
            ...
            try { insert.run(...values); hydrated++; } catch (e) { /* row-level skip */ }
          }
          continue;
        }
